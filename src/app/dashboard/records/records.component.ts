...
import { HealthDocuments, HealthDocumentWrapper } from '@interface/health-document';
...
export class RecordsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._dynamicTitleService.setPageTitle('DigiHeals :: Health Records');
  ...
  getPatientDocument(patientId: string) {
    ...
    this.http.makeAuthRequest<HealthDocumentResponse>('POST', HttpConstants.doctor.getAllDocument + patientId).subscribe(res => {
  ...
  manipulateData() {
    ...
    this.snapRx = this.documents.filter((x) => x.type == "digital_pres" || x.type == "digital");
    this.previewDocuments = this.snapRx;
