...
export class HomeComponent implements OnInit {
  ...
  constructor( private headerService: DynamicHeaderService, private router: Router, private doctorService: DoctorService, private http: BaseHttpService, private patientService: PatientService, private titleService: DynamicTitleService,private geoLocationService:GeoLocationService) {
      }
  ...
  ngOnInit(): void {
    this.titleService.setPageTitle('DigiHeals | Dashboard');
...
getPateintLastRecord(id: string) {
  ...
  this.http.makePartnerRequest<patientLastRecords>({ method: 'POST', url: '/api/v1/digiheals/getLatestPrescriptionForPatient', data: { patientId: id } }).subscribe(res => {
    this.isSpinning = false;
    if (res.hasErrors()) {
...
getUpcomingFollowUp(id: string) {
  ...
  this.http.makeAuthRequest<appointmentData>('POST', HttpConstants.appointment.getFollowAppt, { patientId: id, timestamp: moment().startOf('day').valueOf(), doctorId: this.doctor?.uid || '' }).subscribe(res => {
  this.isSpinning = false;
